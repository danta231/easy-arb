...
sudo mkdir -p /opt/easy-arb /etc/easy-arb /var/lib/easy-arb
...
sudo chmod 750 /etc/easy-tool
```

凭证类变量放入 `/etc/easy-arb/easy-arb-secrets.env` 和 `/etc/easy-tool/easy-tool.env`，不要写入仓库、日志、样例或聊天记录。`easy-arb-secrets.env` 应只允许 root 和 live 运行用户所属组读取，不给 `easytool` 权限。
...
sudo chown root:easyarb /etc/easy-arb /etc/easy-arb/easy-arb-secrets.env
sudo chmod 750 /etc/easy-arb
sudo chmod 640 /etc/easy-arb/easy-arb-secrets.env
...
如果当前服务器实际用 `deploy` 用户直接运行 easy-arb live，而不是 `easyarb` systemd 用户，把上面命令中的 `easyarb` 组替换为 `deploy`，或把 `deploy` 加入只用于读取 easy-arb 密钥的受限组。
...
sudo rsync -a --delete /opt/easy-arb-backups/<previous>/ /opt/easy-arb/
