...
常用环境变量:
  ...
  ARB_RUNTIME_PORTFOLIO_PRIVATE_READONLY_VENUES= # 可选覆盖采集交易所，逗号分隔；留空表示正式接入的 funding-arb 交易所；Gate/Lighter staged 只在显式配置且凭证就绪时采集。
...
PORTFOLIO_PRIVATE_READONLY_DEFAULT_VENUES="binance,bybit,okx,bitget,aster,hyperliquid"
PORTFOLIO_PRIVATE_READONLY_VENUES="${ARB_RUNTIME_PORTFOLIO_PRIVATE_READONLY_VENUES:-${PORTFOLIO_PRIVATE_READONLY_DEFAULT_VENUES}}"
